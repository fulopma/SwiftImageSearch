...
protocol ServiceAPI {
    ...
    func execute<T: Decodable>(request: Request, modelName: T.Type) async throws
        -> T
...
public class ServiceManager: ServiceAPI {
    ...
    private func returnLocalResponse(query: String) -> PixabayResponse? {
        guard let response = localResponses.first(where: { $0.query == query })
        else {
            return nil
        }
    ...
    public func execute<T>(request: any Request, modelName: T.Type) async throws
        -> T where T: Decodable
    {
        if let response = returnLocalResponse(
            query: request.params["q"]?.lowercased() ?? ""
        ) {
    ...
    public func fetchImage(for searchTerm: String, url: String) async throws
        -> Data
    {
    ...
    private func getImageData(for searchTerm: String, with url: String)
        async throws -> Data
    {
        guard let urlObj = URL(string: url) else {
            throw NSError(
                domain: "Invalid URL \(url)",
                code: 1001,
                userInfo: nil
            )
        }
